    ...
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                ...
                <x-input id="userID" class="block mt-1 w-full" type="text" name="userID" :value="old('userID')" required autofocus autocomplete="userID" />
            </div>

            <div class="mt-4">
                <x-label for="role_id" value="{{ __('Rol') }}" />
                <select id="role_id" name="role_id" class="block mt-1 w-full" required autocomplete="off">
                    <option value="">-- Seleccione un rol --</option>
                    <option value="5" {{ old('role_id') == '5' ? 'selected' : '' }}>Docente</option>
                    <option value="6" {{ old('role_id') == '6' ? 'selected' : '' }}>Estudiante</option>
                </select>
            </div>
