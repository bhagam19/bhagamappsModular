        ...
        @forelse ($apps as $app)
            <div class="boton mx-4 my-5 p-3 border rounded shadow-sm text-center"
                ...
                <a href="{{ url($app->ruta) }}" class="text-decoration-none d-block">
                    @if($app->imagen && file_exists(public_path($app->imagen)))
                        <img src="{{ asset($app->imagen) }}" alt="{{ $app->nombre }}"
                            style="height: 64px; width: 64px; margin: 0 auto; border-radius: 15px; border: 1px solid #666; box-shadow: 1px 1px 5px #8a8a8a; transition: all 0.3s ease; display: block;">
                    @else
        ...
        @forelse ($apps as $app)
            <a href="{{ url($app->ruta) }}" class="text-decoration-none">
                <div class="d-flex align-items-center my-2 p-2 border rounded shadow-sm"
                    ...
                    @if($app->imagen && file_exists(public_path($app->imagen)))
                        <img src="{{ asset($app->imagen) }}" alt="{{ $app->nombre }}"
                            style="height: 64px; width: 64px; min-width: 64px; border-radius: 5px; box-shadow: 1px 1px 5px #ebe6ec;">
                    @else
